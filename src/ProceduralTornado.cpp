...
ProceduralTornado::ProceduralTornado(int xDim, int yDim, int zDim, int time) :
	_xDim(xDim), _yDim(yDim), _zDim(zDim)
{
	_data.Set([=]() {
		...
		auto tornadoFlowWithMagnitude = initCStyleArray(new vec4<>[numVectors]);
		transform(tornadoFlow.get(), tornadoFlow.get() + numVectors, 
			tornadoFlowWithMagnitude.get(), Normalizer(0.0, 1.0));
		...
		const float maxLength = minMax.second->w;
		...
		return tornadoFlowWithMagnitude;
	});
}

GLenum ProceduralTornado::getInternalFormat() {
	return GL_RGBA32F;
}

int ProceduralTornado::getWidth() {
	return _xDim;
}

int ProceduralTornado::getHeight() {
	return _yDim;
}

int ProceduralTornado::getDepth() {
	return _zDim;
}

GLenum ProceduralTornado::getFormat() {
	return GL_RGBA;
}

GLenum ProceduralTornado::getType() {
	return GL_FLOAT;
}

shared_ptr<const void> ProceduralTornado::get_data() {
	return _data;
}
